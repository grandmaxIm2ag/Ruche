 ...
 * @author maxence
 */
public class ApprentissageEndGame extends Arbitre implements Runnable{
    ConcurrentHashMap<Integer, Coup> hCoup; 
    int nbSimulations;
    boolean endGame;
    public ApprentissageEndGame(Properties p, String n1, String n2, ConcurrentHashMap<Integer, Coup> h) {
        super(p, n1, n2);
        hCoup = h;
        nbSimulations = 1;
        endGame = false;
    ...
    public void apprentissageEndGame(){
        for(int i=0; i<nbSimulations; i++){
            System.err.println("Nouvelle Partie "+i);
                ...
                endGame=true;
                etat = ATTENTE_COUP;
                joueurs[J1] = new Ordinateur( true,Ordinateur.FACILE_ALEATOIRE, prop, joueurs[J1].pions(), J1, "");
                joueurs[J2] = new Ordinateur( true,Ordinateur.FACILE_ALEATOIRE, prop, joueurs[J2].pions(), J2, "");
                simulation2();
            }
        ...
        if(plateau.estEncerclee(jCourant)){
            ...
            if(aucun){
                ...
                if(joueurs[jCourant] instanceof Ordinateur){
                    ...
                    if(plateau.reine(jCourant)!=null && endGame)
                        hCoup.put(plateau.hashCode(),translateConcreteToAbstract(c, jCourant, plateau));
                    joue(c);
                    }
                }
            }
